...
void cloud_cb (const sensor_msgs::PointCloud2ConstPtr& input)
{
	...
	for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
	{
		...
		for (std::vector<int>::const_iterator pit = it->indices.begin (); pit != it->indices.end (); ++pit)
		{
			...
			//std::cout<<"cluster indices :" <<p.x<< p.y<<p.z<< std::endl;
		...
		*final_cluster = *final_cluster + *cloud_cluster;//sum up all the colored cluster into a complete pc
		...
		//std::cout<<"cluster indices :" <<centroid.x<< centroid.y<<centroid.z<< std::endl;
...
int main (int argc, char** argv)
{
	...
	pub = h.advertise<sensor_msgs::PointCloud2>("/points_cluster",10);
	centroid_pub = h.advertise<lidar_tracker::centroids>("/cluster_centroids",10);
	marker_pub = h.advertise<visualization_msgs::Marker>("centroid_marker",10);
	...
	private_nh.param("is_downsampling", is_downsampling, true);
	...
	ros::Subscriber sub = h.subscribe (points_topic, 10, cloud_cb);
